...
fun roll_left(a,0) =convertToString(a)
    | roll_left(a,1)=
...
val myHashL : (string, string) HashTable.hash_table = 
         HashTable.mkTable (hashFunction, op=) (1, Fail "NOT_FOUND"); 
fun add_elem_right(el:(string * string ),5,right_root,_)= (el,false)
	| add_elem_right(el,_,right_root,true)=(el,true)
	|add_elem_right(el,i,right_root,s)=
	     let 
		 ...
		 in 
			if HashTable.find myHashR ch_i  = NONE then
				if HashTable.find myHashL ch_i <> NONE then ((HashTable.insert myHashR(ch_i,(#1 el)));add_elem_right((ch_i,k),i+1,right_root,true)) else 
				(Queue.enqueue(q_r,(ch_i,k));(HashTable.insert myHashR((ch_i,(#1 el))));add_elem_right(el,i+1,right_root,false))
			else add_elem_right(el,i+1,right_root,false)
		end
		
fun add_elem_left(el:(string * string ),5,left_root,false)= (el,false)
	| add_elem_left(el,_,left_root,true)=(el,true)
	|add_elem_left(el,i,left_root,s)=
	     let 
		 ...
		 in 
			if HashTable.find myHashL ch_i  = NONE then
				if HashTable.find myHashR ch_i <>NONE then ((HashTable.insert myHashL(ch_i,(#1 el)));add_elem_left((ch_i,k),i+1,left_root,true)) else 
				(Queue.enqueue(q_l,(ch_i,k));(HashTable.insert myHashL((ch_i,(#1 el))));add_elem_left(el,i+1,left_root,false))
			else add_elem_left(el,i+1,left_root,false)
		end
 ...
 fun find_right_path(sol_r,m,right_root)=if sol_r=right_root then m else 
  ...
  | bfs((x,s1),(y,s2),right_root,left_root)=
	let 
		...
		val h_l=dequeue(q_l)
	in 
		bfs(add_elem_left(h_l,1,left_root,s2),add_elem_right(h_r,1,right_root,s1),right_root,left_root)
	end	
...
fun validator(str,arg)=if compare(str,arg)=EQUAL then true else 
	let val s=fromList(explode str)
		val s'=rollstring_right(arg,sub(s1,i))
	in 
		vali
