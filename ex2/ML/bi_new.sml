...
fun init_right(q_r,s)=(Queue.clear(q_r);enqueue(q_r,(s,"")))
fun init_left(q_l)= (Queue.clear(q_l);enqueue(q_l,("bgbGgGGrGyry","")))
 ...
 fun find_right_path(sol_r,m,right_root)=if sol_r=right_root then m else 
  ...
  | bfs((x,s1),(y,s2),right_root,left_root)=
	let 
		...
		val h_l=dequeue(q_l)
	in 
		bfs(add_elem_left(h_l,h_l,0,left_root,s2),add_elem_right(h_r,h_r,0,right_root,s1),right_root,left_root)
	end	
                		
fun diapragmateysi(args)=
        let 
                
                val _=HashTable.clear myHashR
                val _=HashTable.clear myHashL
                val _ = init_left(q_l)
