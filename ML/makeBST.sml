...
fun makeBST(L,f)=
        let fun makeTree([],f,T)=T
                ...
                | makeTree(x::xs,f,Node(Empty,data,Empty))=if f(x,data) = true then makeTree(xs,f,Node(Node(Empty,x,Empty),data,Empty)) else makeTree(xs,f,Node(Empty,data,Node(Empty,x,Empty)))
